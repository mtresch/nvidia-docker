...
debianbuster: $(addsuffix -debianbuster, 18.09.2)
...
18.09.2-debianbuster:
	$(DOCKER) build --build-arg VERSION_ID="buster" \
			--build-arg RUNTIME_VERSION="$(RUNTIME_VERSION)+docker18.09.2-1" \
			--build-arg DOCKER_VERSION="docker-ce (= 5:18.09.3~3-0~debian-buster)" \
			--build-arg PKG_VERS="$(VERSION)+docker18.09.2" \
			--build-arg PKG_REV="$(PKG_REV)" \
			-t "nvidia/nvidia-docker2/debian:buster-docker18.09.2" -f Dockerfile.debian .
	$(DOCKER) run --rm -v $(DIST_DIR)/debianbuster:/dist:Z "nvidia/nvidia-docker2/debian:buster-docker18.09.2"
